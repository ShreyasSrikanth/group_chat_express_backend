...
const socket = io('http://3.87.172.140:3000')
...
async function fetchNewUsersForGroup() {
    ...
    let response = await axios.get(`http://3.87.172.140:3000/groups/fetchnewusers?groupId=${groupId}`)
    return response
}
...
async function userGroups() {
    ...
    if (groupName === "") {
        ...
        let response = await axios.post(`http://3.87.172.140:3000/groups/createGroups`, {
            groupName: groupName,
            groupUsers: groups
        }, {
            headers: {
...
async function inviteUsers() {
    try {
        ...
        let response = await axios.post(
            'http://3.87.172.140:3000/groups/addusertogroup',
            {
                groupId: groupId,
                groupUsers: invite
            },
...
async function fetchUserGroup() {
    ...
    let token = localStorage.getItem("token");
    let response = await axios.get(`http://3.87.172.140:3000/groups/fetchgroups`, {
        headers: {
            'Authorization': token
        }
    });
...
async function displayGroupUsers(groupName, groupId) {
    ...
    let response = await axios.get(`http://3.87.172.140:3000/groups/fetchgroupusers?groupId=${groupId}`, {
        headers: {
            'Authorization': token
        }
    });
...
async function storeGroupMessages() {
    ...
    if (selectedFile) {
        ...
        try {
            const response = await axios.post('http://3.87.172.140:3000/groupmessageRoute/filesupload', formData, {
                headers: {
        ...
        let response = await axios.post(`http://3.87.172.140:3000/groupmessageRoute/fetchgroupusers`, {
            message: text,
            groupId: groupId
        }, {
            headers: {
...
async function storeMessagestoBackend() {
    ...
    if (selectedFile) {
        ...
        try {
            const response = await axios.post('http://3.87.172.140:3000/message/filesupload', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': token,
                },
        ...
        if (count === "1") {
            ...
            let response = await axios.post(`http://3.87.172.140:3000/message/storechat`, {
                message: text
            }, {
                headers: {
                    'Authorization': token
...
async function fetchUsers() {
    let token = localStorage.getItem('token');
    let response = await axios.get("http://3.87.172.140:3000/users/fetchusers", {
        headers: {
            'Authorization': token
        }
    })
        ...
        if (normalchats === true) {
            ...
            if (user.id === groupAdmin) {
                ...
                removeButton.addEventListener('click', async () => {
                    let response = await axios.post(`http://3.87.172.140:3000/groups/removegroupuser`, {
                        userId: user.id,
                ...
                /* justify-content: end; */
                /* align-items: end; */
                removeButton.style.left = "82%";
                removeButton.style.height = "2%"
                removeButton.style.fontSize = "xx-small";
                removeButton.style.backgroundColor="darkcyan";
...
async function getAllGroupMessagesfromBackend() {
    try {
        ...
        let response = await axios.get(`http://3.87.172.140:3000/groupmessageRoute/fetchallgroupmessages?groupId=${groupId}`, {
            headers: {
                'Authorization': token
            }
        });
...
async function appendGroupMessage() {
    ...
    let response = await axios.get(`http://3.87.172.140:3000/groupmessageRoute/fetchgroupmessages?groupId=${groupId}`, {
        headers: {
            'Authorization': token
        }
    });
...
async function getMessagesfromBackend() {
    try {
        let token = localStorage.getItem("token");
        let response = await axios.get(`http://3.87.172.140:3000/message/getmessages`, {
            headers: {
                'Authorization': token
            }
        });
...
async function appendNewMessage() {
    let token = localStorage.getItem("token");
    let response = await axios.get(`http://3.87.172.140:3000/message/getnewmessage`, {
        headers: {
            'Authorization': token
        }
    });
